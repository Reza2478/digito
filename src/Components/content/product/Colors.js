    ...
    <div className="mb-5 flex w-full items-center justify-evenly md:justify-start">
      ...
      <div className="flex items-center md:mr-5">
        <div className="flex items-center justify-center">
          {colors.map((item, id) => (
            <span onClick={() => setChoice(id)} key={id} className={`${choice === id && `ring-2 `} ml-[-4px] flex h-5 w-5 cursor-pointer items-center justify-center rounded-full ${item} border border-white  sm:h-7 sm:w-7`}>
              {choice === id && (
