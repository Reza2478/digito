  ...
  return (
    <div className="mb-2 flex items-center justify-between rounded-md bg-white p-2 shadow-md">
      ...
      <div className="flex flex-col items-end gap-y-8">
        <svg onClick={() => dispatch({ type: "REMOVE", payload: product })} xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 cursor-pointer" viewBox="0 0 20 20" fill="currentColor">
          <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
        </svg>
        <div className="flex items-center justify-between gap-x-2">
          <div onClick={() => dispatch({ type: "INCREASE", payload: product })} className="flex h-5 w-5 items-center justify-center rounded-full bg-gray-300 text-slate-800">
          ...
          {product.quantity > 1 ? (
            <div onClick={() => dispatch({ type: "DECREASE", payload: product })} className="flex h-5 w-5 items-center justify-center rounded-full bg-orange-100 text-orange-500">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 cursor-pointer" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" clipRule="evenodd" />
              </svg>
            </div>
          ) : (
            <div onClick={() => dispatch({ type: "REMOVE", payload: product })} className="flex h-5 w-5 items-center justify-center rounded-full bg-orange-100 text-orange-500">
